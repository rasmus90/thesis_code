...
class Process_Valid_Range():
    def __init__(self, valid_range, time, load_val, title_):
        ...
        self.title_ = title_
        self.path = "/home/rasmus/master/Field Experiment/Experiments/February/28.02/Load Cell Data/Cold 1300 50/"
    ...
    def plot_valid_range(self, show, save, name):
        #import scipy.signal
        #sgf = scipy.signal.savgol_filter(self.v_load_val, 1001, 5)
        ...
        #plt.plot(self.v_time/1000., sgf)
        plt.axis([self.v_time[0]/1000, self.v_time[-1]/1000, 0, 15])
    ...
    def save_to_np(self, path):
        np.save(path+"load_values.npy", self.savitzky_golay(self.v_load_val, 1001, 5))
        np.save(path+"load_time.npy", self.v_time/1000.)

    def get_mean(self):
        # print "Mean from selected range: {:4.1f}".format(np.mean(self.v_load_val))
        return round(np.mean(self.v_load_val), 2)

    def get_std(self):
        # print "Std: {:4.3f}".format(np.std(self.v_load_val))
        return round(np.std(self.v_load_val, ddof=1), 3)

    def get_mu(self, normal_load):
        # print "u = {:4.3f}".format(np.mean(self.v_load_val)/normal_load)
        return round(np.mean(self.v_load_val)/normal_load, 4)
    ...
    def measurement_section(self):
        ...
        """
        sg = self.savitzky_golay(self.v_load_val, 1001, 5)
        plt.plot(self.v_time/1000., sg * 9.81, linewidth=3.0)
        plt.xlabel("Time (seconds)", fontsize=15)
        plt.ylabel("Pull Force $F_P$ (N)", fontsize=15)
        plt.axvspan(20, 24.6, facecolor='0.5', alpha=0.4)
        plt.title("Load cell data from a sliding test.")
        plt.text(12, 100, r'$F_F=F_P$', fontsize=20)
        plt.annotate(r'$\mu_k = \frac{\bar{F_F}}{F_N}$', xy=(22.5, 19), xytext=(12, 70),
